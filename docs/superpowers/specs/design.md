...
│   │   ├── Components/
│   │   │   ├── Layout/
│   │   │   │   ├── MainLayout.razor          # MudBlazor 深色主题配置
│   │   │   │   └── NavMenu.razor             # 侧边导航
│   │   │   └── Pages/
│   │   │       ├── Dashboard.razor           # 首页：状态总览
│   │   │       ├── Games.razor               # 游戏库：卡片网格 + 搜索/筛选 + 全局同步
│   │   │       ├── GameDetail.razor          # 游戏详情：成就列表（路由 /games/{Id}）
│   │   │       ├── Settings.razor            # 账号设置 + API Key + 语言 + 定时同步
│   │   │       └── Login.razor               # Steam 账号登录页
│   │   ├── Middleware/
...
│   │   ├── Models/
│   │   │   ├── Game.cs                       # 游戏实体（含状态 + 进度 + i18n）
│   │   │   ├── Achievement.cs                # 成就实体（含调度 + 缓存数据 + i18n）
│   │   │   └── SteamConfig.cs                # Steam 账号配置实体（含语言偏好）
│   │   └── Dto/
...
| 表名 | EF Core 实体 | 主要字段 |
|------|-------------|---------|
| `steam_config` | `SteamConfig` | `Username`, `PasswordEnc`, `WebApiKey`, `SessionToken`, `SyncCron`, `Language` |
| `game` | `Game` | `AppId`, `Name`, `NameI18n`, `TargetHours`, `Status`, `TotalPlayMinutes`, `ReferencePlayMinutes`, `AchievementsCachedAt` |
| `achievement` | `Achievement` | `GameId`, `AppId`, `ApiName`, `DisplayName`, `DisplayNameI18n`, `GlobalPercent`, `IconUrl`, `IconGrayUrl`, `UnlockOffsetMinutes`, `IsUnlocked`, `UnlockedAt` |
...
1. Steam Web API: GetSchemaForGame(appid, language)
   → 成就列表、DisplayName（含本地化）、IconUrl、IconGrayUrl
...
4. UPSERT achievement 表（按 game_id + api_name 唯一键）
   → 多语言：language=="english" 写 DisplayName + 清空 DisplayNameI18n
              language!="english" 写 DisplayNameI18n + 保留 DisplayName
...
**成就解锁：** 通过 SteamKit2 发送 `EMsg.ClientStoreUserStats2`（参考 ASFAchievementManager）。

### 5.7 SyncBackgroundService
...
Step 2: 遍历所有游戏，逐一调用 AchievementDataService.LoadAchievementsAsync
  → 等价于对每个游戏执行一次 GameDetail 的 Refresh 操作
  → 实时更新进度条（0→100%）和状态文本
```
...
### 7.3 GameDetail（游戏详情）— 路由 `/games/{Id}`
...
| 名称 | `DisplayNameI18n ?? DisplayName`（主） + `ApiName`（副，灰色小字） |
...
- **语言 / Language**：下拉选择 `English` 或 `简体中文`，保存到 `steam_config.language`；下次 Refresh / Sync Now 时按此语言拉取本地化名称
- **Background Sync 配置：**
  - 5-field Cron 表达式，默认 `0 0 * * *`，带实时语法校验和下次执行时间预览
  - **Sync Now** 按钮：立即触发完整同步（库 + 所有游戏成就）
